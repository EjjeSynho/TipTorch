...
force_positive_coef = lambda x: torch.clamp(-x, min=0).pow(2).mean()
...
class MUSEObservation:
    def __init__(self, raw_path, cube_path, cache_path):
        ...
        self.canvas_sparse = torch.zeros([self.N_wvl, self.cube_sparse.shape[-2], self.cube_sparse.shape[-1]], device=device)
    ...
    def _load_data(self):
        ...
        spectral_cubes, spectral_info, data_cache, model_config = LoadCachedDataMUSE(self.raw_path, self.cube_path, self.cache_path, save_cache=True, device=device, verbose=True)   
    ...
    def AddSourcesToModelConfig(self):
        ...
        self.model_config['sources_science']['Zenith']  = torch.tensor(sources_zenith,  device=device).unsqueeze(-1)
        self.model_config['sources_science']['Azimuth'] = torch.tensor(sources_azimuth, device=device).unsqueeze(-1)
    ...
    def InitSimulation(self) -> None:
        ...
        self.model_config['sources_science']['Wavelength'] = torch.tensor(self.λ_sparse, device=device, dtype=torch.float32) * 1e-9 #[m]
        self.model_config['telescope']['PupilAngle']       = torch.tensor(self.model_config['telescope']['PupilAngle'], device=device)
    ...
    def _get_flux_factor(self, quasi_inf_PSF_size: int = 511) -> None:
        ...
        core_mask_inf   = torch.tensor(mask_square(quasi_inf_PSF_size, self.flux_core_radius+1)[None,None,...], dtype=default_torch_type, device=device)
    ...
    def _compute_fitting_weights(self) -> FitWeights:
        ...
        src_fluxes  = torch.tensor(self.sources.table['peak_value'].to_numpy(), device=device, dtype=torch.float32)
        src_weights = torch.tensor(self.sources.table['weight'].to_numpy(),     device=device, dtype=torch.float32)
        ...
        return FitWeights(
            ...
            total      = w_total,
            LO         = 1e-7 if not suppress_LO_flag   else 1e3,
            phase_bump = 5e-5 if not suppress_bump_flag else 1e3,
            positive   = 2.0,
            unit_flux  = 0.2
        )
    ...
    def _loss_LO(self, fit_weights: FitWeights):
        ...
        phase_bump_positive = force_positive_coef(self.PSF_model.inputs_manager['LO_coefs'][:, 0]) * fit_weights.phase_bump
        # Force defocus to be positive to mitigate sign ambiguity
        first_defocus_penalty = force_positive_coef(self.PSF_model.inputs_manager['LO_coefs'][:, 2]) * fit_weights.LO  #NOTE: won't work with the chromatic defocus
    ...
    def FitPSFModel(self, repeat=2, max_iter=200, return_params=False) -> Optional[torch.Tensor]:
        ...
        # Return the normalized vector of model parameters if needed
        if return_params:
            return x_params
    ...
    def SimulateField(self, full_spectrum=False) -> None:
        ...
        if full_spectrum:
            ...
            self.simulated_sparse = self.simulate_sparse(x_params, src_subset=None)
    ...
    def PlotSourceSpectra(self, title='Sources spectra', show_sparse=True, plot_residual=False, smooth_kernel=None, figsize=(10, 6)):
        """
        ...
        if plot_residual:
            ...
            src_spectra_full   = self.sources.spectra_res_full
            src_spectra_sparse = self.sources.spectra_res_sparse
        else:
            src_spectra_full   = self.sources.spectra_full
            src_spectra_sparse = self.sources.spectra_sparse
        ...
        for i_src in range(N):
            plt.plot(self.λ_full, src_spectra_full[i_src], linewidth=0.25, alpha=(0.5 if show_smooth else 0.8), color=colors[i_src], label=f'Source {i_src+1} (full spectrum)')
            if show_sparse:
                plt.scatter(self.λ_sparse, src_spectra_sparse[i_src].cpu().numpy(),
            ...
            if show_smooth:
                ...
                plt.plot(self.λ_full, spectrum_smooth, linewidth=0.65, alpha=1, color=colors[i_src], label=f'Source {i_src+1}')
...
ob = MUSEObservation(raw_path, cube_path, cache_path)
...
#%%
x_params = ob.FitPSFModel(repeat=3, max_iter=200, return_params=True)
...
ob.PlotSourceSpectra(title='Sources spectra (residual)', show_sparse=False, plot_residual=True, smooth_kernel=5)
