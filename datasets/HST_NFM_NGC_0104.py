#%%
# HUGS NGC 0104 processing follows the NGC 6441 routine:
...
def load_hugs_catalog(method=1):
    catalog_path = data_folder / f"hlsp_hugs_hst_wfc3-uvis-acs-wfc_ngc0104_multi_v1_catalog-meth{method}.txt"
    df = pd.read_csv(
        catalog_path,
        comment="#",
        sep=r"\s+",
...
def load_hacks_pm_catalog():
    pm_path = data_folder / "hlsp_hacks_hst_acs-wfc3_ngc0104_multi_v1.0_pm-catalog.fits"
...
def select_reduced_cube(center_coord, max_sep=MATCH_RADIUS):
    ...
    if sep > max_sep:
        raise ValueError(
            f"Nearest MUSE cube is {cube_name}, {sep.to(u.arcmin):.2f} from NGC 0104. "
            f"Provide an NGC 0104 cube in {CUBES_FOLDER} or set reduced_cube explicitly."
        )
...
# Leave as None to auto-select a reduced cube whose pointing lies near NGC 0104.
reduced_cube = None
...
image_path = data_folder / "hlsp_hugs_hst_acs-wfc_ngc0104_f435w_v1_stack-0100s.fits"
...
if df_sel.empty:
    raise ValueError(f"No HUGS NGC 0104 sources found within {radius:.2f} of {cube_path.name}")
...
ax.set_title(f"NGC 0104 sources near {cube_path.name}")
plt.tight_layout()
plt.show()
...
plt.title("NGC 0104 HST sources aligned to MUSE detections")
plt.legend()
plt.grid(alpha=0.3)
plt.show()
