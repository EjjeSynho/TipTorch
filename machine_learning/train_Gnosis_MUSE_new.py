...
wavelength_selected = wavelength  # Store reference to full wavelength array
...
#%%
# Initialize PSFModelNFM wrapper
PSF_model = PSFModelNFM(
    ...
    Moffat_absorber = predict_Moffat,
    device          = device
)
...
#%%
# Define which parameters to predict and which to fix
all_entries   = ['r0', 'F', 'dn', 'dx', 'dy', 'bg', 'J', 'Jxy'] + \
                (['LO_coefs'] if LO_NCPAs else []) + \
                (['amp', 'b', 'alpha', 'beta', 'ratio', 'theta'] if predict_Moffat else [])

fixed_entries = ['dx', 'dy', 'Jxy', 'bg']
predicted_entries = ['r0', 'F', 'dn', 'J']

if LO_NCPAs:       predicted_entries += ['LO_coefs']
if predict_Moffat: predicted_entries += ['amp', 'b', 'alpha']

# Get output size based on predicted entries (from inputs manager)
PSF_model.inputs_manager.set_optimizable(predicted_entries, True)
PSF_model.inputs_manager.set_optimizable(list(set(all_entries) - set(predicted_entries)), False)
output_size = PSF_model.inputs_manager.stack().shape[-1]
...
net = Gnosis(batch_init['NN input'].shape[1], output_size, 200, 0.1)#0.25)
net.to(device)
net.float()
...
#%%
def run_model(PSF_model, config, predicted_inputs, fixed_inputs={}):
    ...
    x_unpacked = predicted_inputs | fixed_inputs
    
    return PSF_model(x_unpacked)


def func(x_, config, fixed_inputs):
    # Unstack neural network output to get predicted parameters
    pred_inputs = PSF_model.inputs_manager.unstack(net(x_), include_all=False, update=True)
    return run_model(PSF_model, config, pred_inputs, fixed_inputs)
...
for epoch in range(epochs):
    ...
    for i, id in enumerate(train_ids):
        ...
        PSF_pred = run_model(PSF_model, config, PSF_model.inputs_manager.unstack(net(x), include_all=False, update=True), fixed_inputs)
    ...
    for i, id in enumerate(val_ids):
        with torch.no_grad():
            ...
            PSF_pred = run_model(PSF_model, config, PSF_model.inputs_manager.unstack(net(x0), include_all=False, update=True), fixed_inputs)
            loss = loss_combined(PSF_pred, PSF_0)
...
def get_TT_jitter(r0, WFS_nPix, WFS_Nph, WFS_psInMas, WFS_RON):
    ...
    D_telescope = PSF_model.model.D  # Use the model's telescope diameter
    jitter_STD = lambda TT_WFE: np.arctan(2*TT_max/D_telescope * TT_WFE) * rad2mas # [mas], TT_WFE in [m]
...
if direct_tuned:
    ...
    if predict_Moffat:
        inputs_direct['amp'] = torch.zeros([1])
else:
    inputs_direct = {
...
with torch.no_grad():
    for i in tqdm(val_ids):
        ...
        pred_inputs_stats.append(PSF_model.inputs_manager.unstack(net(x0), include_all=False, update=True))
        ...
        PSFs_2_val_poly.append(run_model(PSF_model, config, inputs_direct_).cpu())
...
# Collect Moffat stats if applicable
if predict_Moffat:
...
# Collect LO NCPA stats if applicable
if LO_NCPAs:
    LO_stats    = np.array( [x.tolist() for pred_inputs in pred_inputs_stats for x in pred_inputs['LO_coefs']] )

# Create grid plot for statistics
n_plots = 8 if predict_Moffat else 5
...
sns.histplot(dn_stats, fill=True, label=r'$\Delta n$', ax=ax[0,1], zorder=-1, element='step', alpha=0.4)

# Plot Moffat parameters if applicable
if predict_Moffat:
...
# Plot LO stats if applicable  
if LO_NCPAs:
    # Plot first few LO coefficients as example
    for j in range(min(3, LO_stats.shape[1])):
        sns.histplot(LO_stats[:, j], fill=True, ax=ax[0,3], alpha=0.3, element='step')
    ax[0,3].set_xlabel('LO coefs (first 3)')
else:
    ax[0,3].axis('off')
