...
#%%
toy_MUSE = TipToyMUSE(config_file, obs_info, device=device)

r0  = torch.tensor(obs_info['SPTR0'], requires_grad=True,  device=toy_MUSE.device)
L0  = torch.tensor(obs_info['SPTL0'], requires_grad=False, device=toy_MUSE.device)
...
for rid in range(4):
    ...
    for _ in tqdm(range(20)):
        ...
        for wvl in data_cube.wavelengths:
            ...
            toy_MUSE = TipToyMUSE(config_file_1, obs_info_1, norm_regime=None, device=device)
            ...
            toy_MUSE.wind_speed = torch.tensor([V_rand, V_rand]).to(device)

            PSF_1 = toy_MUSE.PSD2PSF(*[r0, L0, F, dx, dy, bg, n, Jx, Jy, Jxy])
            PSF_DL = toy_MUSE.DLPSF()
