...
def loss_LO_fn():
    if isinstance(PSF_model.LO_basis, PixelmapBasis):
        ...
        LO_loss = PSF_model.inputs_manager['LO_coefs'].abs().sum()**2 * 1e-7
        # Constraint to enforce first element of LO_coefs to be positive
        first_coef_penalty = torch.clamp(-PSF_model.inputs_manager['LO_coefs'][0, 0], min=0).pow(2) * 5e-5
...
def minimize_params(loss_fn, include_list, exclude_list, max_iter, verbose=True):
    ...
    if result['nit'] < max_iter * 0.3 or result['fun'] > 1:
        ...
        PSF_model.inputs_manager.unstack(x_backup, include_all=True, update=True)
        result = lim_lambda('bfgs', 1e-5)

    OPD_map = PSF_model.OPD_func().detach().cpu().numpy() if PSF_model.LO_NCPAs else None
