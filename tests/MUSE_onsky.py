...
#%%
# wvl_ids = np.clip(np.arange(0, (N_wvl_max:=31)+1, 5), a_min=0, a_max=N_wvl_max-1)
# wvl_ids = np.clip(np.arange(0, (N_wvl_max:=31)+1, 3), a_min=0, a_max=N_wvl_max-1)
wvl_ids = np.clip(np.arange(0, (N_wvl_max:=31)+1, 2), a_min=0, a_max=N_wvl_max-1)
# wvl_ids = np.clip(np.arange(0, (N_wvl_max:=31)+1, 2), a_min=0, a_max=N_wvl_max-1)[:14]
...
PSF_model_backup = PSF_model.copy()

#%%
PSF_model = PSF_model_backup.copy()
PSF_test = PSF_model()

#%%
PSF_test = PSF_model_backup()

#%%
PSF_model.model.Update(grids=True, pupils=True, tomography=True)

# PSF_model.model.InitValues()
PSF_test = PSF_model()

#%%
with torch.no_grad():
    # TODO: add setting to wvl_ctrl so crop factor can be represented with splines
    PSD_before = {k: v.detach().clone() for k, v in PSF_model_backup.model.PSDs.items()}
     # The actual size of the simulated PSFs
    current_PSF_size = PSF_model.model.N_pix
    PSF_small = PSF_model()
    # Quasi-infinite PSF image to compute how much flux is lost while cropping
    PSF_model.SetImageSize(quasi_inf_PSF_size := 511)
    PSF_big = PSF_model()
    PSF_model.SetImageSize(current_PSF_size)
    # PSF_test = func(x0) # Update internal states
    PSF_test = PSF_model()
    # torch.cuda.empty_cache()
    PSD_after = {k: v.detach().clone() for k, v in PSF_model.model.PSDs.items()}

# flux_correction = (PSF_big.amax(dim=(-2,-1)) / PSF_small.amax(dim=(-2,-1))).squeeze()
...
# Compare all inuts_managers params between PSF_model and PSF_model_backup
for k in PSF_model.inputs_manager.parameters:
    diff_param = (PSF_model.inputs_manager[k] - PSF_model_backup.inputs_manager[k]).abs().mean().item()
    print(f"Mean absolute difference in parameter '{k}' before and after size reset: {diff_param:.3e}")



#%%
# Compare PSF_before and PSF_after PSDs
for k in PSD_before.keys():
    diff_PSD = (PSD_before[k] - PSD_after[k]).abs().mean().item()
    print(f"Mean absolute difference in PSD '{k}' before and after size reset: {diff_PSD:.3e}")


#%%
# aaa = (PSF_1 - PSF_small).abs().mean().item()
# print(f"Difference between PSF before and after size reset: {aaa:.3e}")

draw_PSF_stack(
    PSF_test.cpu().numpy()[id_src, wvl_select, ...],
    PSF_1.cpu().numpy()[id_src, wvl_select, ...],
    average=True,
    min_val=vmin,
    max_val=vmax,
